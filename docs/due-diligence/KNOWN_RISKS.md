...
| 9 | AVG-flows (inzage/export/verwijdering, retentie, consent-UI) | **Grotendeels opgelost**: zelfbediening op `/instellingen/privacy` (inzage, JSON-export, verwijdering-als-anonimisering) via `src/server/privacy.ts`, bewaartermijnen + `scripts/retention.mjs`. Nog open: consent-intrekken-UI (niet gevonden in `app/**`) en een cron voor de retentie (PRIVACY.md). |
